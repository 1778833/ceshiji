document.addEventListener('DOMContentLoaded', () => {
    ...
    const screen = document.getElementById('screen'); // 获取手机屏幕元素
    ...
    // 状态变量
    let currentEditingElement = null; // 当前正在编辑图片的元素
    let currentEditingId = null; // 当前正在编辑的项ID (用于保存)
    let currentEditingType = null; // 'app', 'photo', 'widget', 'bar'
    ...
    function enableEditMode() {
        ...
        // 进入编辑模式时，立即冻结布局，防止拖拽单个元素导致其他元素错位
        freezeLayout();
    ...
    document.addEventListener('pointerdown', (e) => {
        ...
        if (isEditMode) {
            ...
            // 此时所有元素都已经是 absolute 了，直接读取 left/top
            // 注意：style.left 可能是百分比（如果刚加载）或像素（如果刚冻结）
            // 为了拖拽顺滑，我们获取当前 computed 的像素值
    ...
    function loadAllSettings() {
        ...
        if (savedBg) {
            screen.style.backgroundImage = `url(${savedBg})`;
        ...
        const appData = JSON.parse(localStorage.getItem('fruit-machine-apps')) || {};
        const iconData = JSON.parse(localStorage.getItem('fruit-machine-icons')) || {}; 
        ...
        const photoData = JSON.parse(localStorage.getItem('fruit-machine-photos')) || {};
    ...
    document.querySelectorAll('.app-name').forEach(nameSpan => {
        nameSpan.addEventListener('dblclick', (e) => {
            if(isEditMode) return; 
            e.preventDefault();
            e.stopPropagation();
            const appId = nameSpan.closest('.app-icon').id;
            const currentName = nameSpan.textContent;
            const newName = prompt('输入新的应用名称:', currentName);
            if (newName && newName.trim() !== '') {
                nameSpan.textContent = newName;
                saveAppData(appId, { name: newName });
            }
        });
    });
