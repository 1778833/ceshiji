 ...
 * @returns {Promise<string>} - 返回压缩后的 Base64 数据 URL
 */
window.compressImage = function(file, maxWidth = 1920, maxHeight = 1920, quality = 0.8) {
...
/**
 * 压缩 Base64 图片字符串
 * @param {string} base64 - 原 Base64
 * @param {number} maxWidth - 最大边长限制
 * @param {number} quality - 压缩质量
 * @returns {Promise<string>}
 */
window.compressBase64 = function(base64, maxWidth = 1080, quality = 0.8) {
    return new Promise((resolve) => {
        if (!base64 || typeof base64 !== 'string' || !base64.startsWith('data:image')) {
            resolve(base64);
            return;
        }
        // 如果 base64 长度小于 100KB，可能没必要压缩，避免阻塞 UI
        ...
        img.onload = () => {
            ...
            if (width > maxWidth || height > maxWidth) {
                const ratio = Math.max(width / maxWidth, height / maxWidth);
            ...
            let newData = canvas.toDataURL('image/webp', quality);
            // Fallback or check size
            if (newData.length >= base64.length) {
                newData = canvas.toDataURL('image/jpeg', quality);
            }
            
            resolve(newData.length < base64.length ? newData : base64);
        };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    fileInput.addEventListener('change', async (e) => {
        ...
        try {
            const resultUrl = await window.compressImage(file, 800, 800, 0.8);
