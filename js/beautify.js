...
.feshuiguo #bar-regex      {}`;
    ...
    exportConfigBtn.addEventListener('click', () => {
        ...
        downloadAnchorNode.download = "fruit-machine-full-backup.json";
        ...
        input.onchange = e => {
            ...
            reader.onload = readerEvent => {
                try {
                    ...
                    // 新增：
                    // 如果有 layout 则设置，否则移除以恢复默认
                    try {
                        if(config.layout) {
                            localStorage.setItem('fruit-machine-layout', JSON.stringify(config.layout));
                        } else {
                            localStorage.removeItem('fruit-machine-layout');
                        }
